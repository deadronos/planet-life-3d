...
export function parseRuleDigits(digits: unknown): boolean[] {
  const s = typeof digits === 'string' ? digits : digits == null ? '' : String(digits)
...
export class LifeSphereSim {
  ...
  constructor(opts: {
    ...
    this.normals = new Array(this.cellCount)
    this.positions = new Array(this.cellCount)
  ...
  pointToCell(point: THREE.Vector3): { lat: number; lon: number } {
    ...
    const lon = ((Math.round(lonT * this.lonCells) % this.lonCells) + this.lonCells) % this.lonCells
    return { lat, lon }
  }

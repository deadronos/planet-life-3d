...
import { BUILTIN_PATTERN_NAMES, getBuiltinPatternOffsets, parseAsciiPattern } from '../sim/patterns'
...
export function PlanetLife() {
  ...
  const params = useControls(() => {
    ...
    return {
      ...
      Rendering: folder(
        {
          ...
          cellRenderMode: { value: 'Texture' as const, options: ['Texture', 'Dots', 'Both'] as const },
      ...
      Seeding: folder(
        {
          seedMode: { value: 'set' as const, options: ['set', 'toggle', 'clear', 'random'] as const },
...
`.trim()
  ...
  const seedAtPoint = useCallback((point: THREE.Vector3) => {
    const sim = simRef.current
    if (!sim) return
  ...
  const onMeteorImpact = useCallback((id: string, impactPoint: THREE.Vector3) => {
